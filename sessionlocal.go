	...
	for cmd != rtcpStopService {
		...
		case <-ticker.C:
			...
			if rc != nil {
				...
				ti, td := rtcpInterval(
					outActive+inActive,
					int(rs.activeSenders),
					rs.RtcpSessionBandwidth,
					rs.avrgPacketLength,
					rs.weSent,
					false,
				)
...
func (rs *Session) sendDataCtrlEvent(code int, ssrc, index uint32) {
	...
	if ctrlEvArr[0] != nil {
		select {
		case ch <- ctrlEvArr[:]: // send control event
		default:
		}
	}
}
